...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is GameplayLogicNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (GameplayLogicNetworkObject.Length > 0 && GameplayLogicNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(GameplayLogicNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<GameplayLogicBehavior>();
						}
					}
			...
			else if (obj is TrainNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (TrainNetworkObject.Length > 0 && TrainNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(TrainNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<TrainBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateGameplayLogic instead, its shorter and easier to type out ;)")]
		public GameplayLogicBehavior InstantiateGameplayLogicNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(GameplayLogicNetworkObject[index]);
			var netBehavior = go.GetComponent<GameplayLogicBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<GameplayLogicBehavior>().networkObject = (GameplayLogicNetworkObject)obj;
		...
		[Obsolete("Use InstantiateTrain instead, its shorter and easier to type out ;)")]
		public TrainBehavior InstantiateTrainNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(TrainNetworkObject[index]);
			var netBehavior = go.GetComponent<TrainBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<TrainBehavior>().networkObject = (TrainNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of GameplayLogic
		/// </summary>
		/// <returns>
		/// A local instance of GameplayLogicBehavior
		/// </returns>
		/// <param name="index">The index of the GameplayLogic prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public GameplayLogicBehavior InstantiateGameplayLogic(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(GameplayLogicNetworkObject[index]);
			var netBehavior = go.GetComponent<GameplayLogicBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of Train
		/// </summary>
		/// <returns>
		/// A local instance of TrainBehavior
		/// </returns>
		/// <param name="index">The index of the Train prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public TrainBehavior InstantiateTrain(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(TrainNetworkObject[index]);
			var netBehavior = go.GetComponent<TrainBehavior>();
